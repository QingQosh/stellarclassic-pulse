...
// `stellarclassic_pulse_counter_overflow_total` metric the moment it saturates so
...
pub fn record_counter_overflow_detected(counter_name: &str) {
    m::counter!(
        "stellarclassic_pulse_counter_overflow_total",
...
pub fn record_counter_state(counter_name: &str, value: u64) {
    m::gauge!(
        "stellarclassic_pulse_counter_state",
        "counter" => counter_name.to_string()
    )
    .set(value as f64);
}
    ...
    PrometheusBuilder::new()
        .set_buckets_for_metric(
            metrics_exporter_prometheus::Matcher::Full(
                "stellarclassic_pulse_http_request_duration_seconds".to_string(),
            ),
...
pub fn record_events_indexed(count: u64) {
    m::counter!("stellarclassic_pulse_events_indexed_total").increment(count);
}

/// Update the current ledger being processed
pub fn update_current_ledger(ledger: u64) {
    m::gauge!("stellarclassic_pulse_indexer_current_ledger").set(ledger as f64);
}

/// Update the latest ledger from RPC
pub fn update_latest_ledger(ledger: u64) {
    m::gauge!("stellarclassic_pulse_indexer_latest_ledger").set(ledger as f64);
}

/// Update the indexer lag
pub fn update_indexer_lag(lag: u64) {
    m::gauge!("stellarclassic_pulse_indexer_lag_ledgers").set(lag as f64);
}

/// Update the last checkpointed ledger
pub fn update_checkpoint_ledger(ledger: u64) {
    m::gauge!("stellarclassic_pulse_indexer_checkpoint_ledger").set(ledger as f64);
}

/// Update the age of table statistics (seconds since last ANALYZE)
pub fn update_stats_age_seconds(age_secs: u64) {
    m::gauge!("stellarclassic_pulse_stats_last_analyzed_age_seconds").set(age_secs as f64);
}
...
pub fn record_indexer_is_leader(is_leader: bool) {
    m::gauge!("stellarclassic_pulse_indexer_is_leader").set(if is_leader { 1.0 } else { 0.0 });
}

/// Record an RPC error
pub fn record_rpc_error() {
    m::counter!("stellarclassic_pulse_rpc_errors_total").increment(1);
}

/// Record a validation failure
pub fn record_validation_failure() {
    m::counter!("stellarclassic_pulse_events_validation_failed_total").increment(1);
}

/// Record an oversized event that was skipped due to exceeding MAX_EVENT_DATA_BYTES.
pub fn record_oversized_event() {
    m::counter!("stellarclassic_pulse_events_oversized_total").increment(1);
}

/// Record a duplicate event
pub fn record_duplicate_event() {
    m::counter!("stellarclassic_pulse_events_duplicate_total").increment(1);
}

/// Record an XDR validation failure (issue #267)
pub fn record_xdr_invalid() {
    m::counter!("stellarclassic_pulse_events_xdr_invalid_total").increment(1);
}

/// Record an invalid contract ID (issue #370)
pub fn record_invalid_contract_id() {
    m::counter!("stellarclassic_pulse_events_invalid_contract_id_total").increment(1);
}

/// Record a passed XDR validation (issue #616)
pub fn record_xdr_validation_pass() {
    m::counter!("stellarclassic_pulse_xdr_validation_pass_total").increment(1);
}

/// Record a failed XDR validation with a field label (issue #616)
pub fn record_xdr_validation_fail(field: &str) {
    m::counter!(
        "stellarclassic_pulse_xdr_validation_fail_total",
...
pub fn record_web_push_sent() {
    m::counter!("stellarclassic_pulse_web_push_sent_total").increment(1);
}

/// Issue #839: Record a Web Push notification failure.
pub fn record_web_push_failed() {
    m::counter!("stellarclassic_pulse_web_push_failed_total").increment(1);
}
...
pub fn record_push_delivery_latency(device_type: &str, duration: std::time::Duration) {
    m::histogram!(
        "stellarclassic_pulse_push_delivery_latency_seconds",
        "device_type" => device_type.to_string()
    )
...
pub fn update_pool_utilization(pool: &PgPool, max_connections: u32) {
    ...
    m::gauge!("stellarclassic_pulse_db_pool_utilization").set(utilization);
    m::gauge!("stellarclassic_pulse_db_pool_active_connections").set(active);
    m::gauge!("stellarclassic_pulse_db_pool_max_connections").set(max);
}

/// Issue #622: Record connection acquisition latency.
pub fn record_pool_acquire_latency(duration: std::time::Duration) {
    m::histogram!("stellarclassic_pulse_db_pool_acquire_latency_seconds")
        .record(duration.as_secs_f64());
}

/// Issue #622: Record a pool exhaustion event (utilization >= 90%).
pub fn record_pool_exhaustion_alert() {
    m::counter!("stellarclassic_pulse_db_pool_exhaustion_alerts_total").increment(1);
}

/// Record a full-text search query duration
pub fn record_search_query_duration(duration: std::time::Duration) {
    m::histogram!("stellarclassic_pulse_search_query_duration_seconds").record(duration.as_secs_f64());
}

/// Increment the contract count cache invalidation counter
pub fn record_contract_count_cache_invalidation() {
    m::counter!("stellarclassic_pulse_contract_count_cache_invalidations_total").increment(1);
}
...
pub fn update_contract_count_cache_hit_ratio(hits: u64, misses: u64) {
    ...
    m::gauge!("stellarclassic_pulse_contract_count_cache_hit_ratio").set(ratio);
}

/// Record a Lua script timeout
pub fn record_lua_timeout() {
    m::counter!("stellarclassic_pulse_lua_timeout_total").increment(1);
}

pub fn record_replay_job() {
    m::counter!("stellarclassic_pulse_replay_jobs_total").increment(1);
}

/// Record a feature flag auto-rollback event (#587)
pub fn record_feature_flag_rollback(flag_name: &str) {
    m::counter!(
        "stellarclassic_pulse_feature_flag_rollback_total",
...
pub fn record_migrations_applied(count: u64) {
    m::counter!("stellarclassic_pulse_migrations_applied_total").increment(count);
}

/// Set the gauge tracking the highest applied migration version (issue #411)
pub fn set_last_migration_version(version: i64) {
    m::gauge!("stellarclassic_pulse_last_migration_version").set(version as f64);
}

/// Record events pruned
pub fn increment_events_pruned(count: u64) {
    m::counter!("stellarclassic_pulse_events_pruned_total").increment(count);
}

/// Record events deleted (GDPR right-to-erasure)
pub fn record_events_deleted(count: u64) {
    m::counter!("stellarclassic_pulse_events_deleted_total").increment(count);
}
...
pub fn record_http_request_duration(
    ...
    m::histogram!(
        "stellarclassic_pulse_http_request_duration_seconds",
...
pub fn record_schema_validation_fail(contract_id: &str) {
    m::counter!(
        "stellarclassic_pulse_schema_validation_fail_total",
        "contract_id" => contract_id.to_string()
    )
    .increment(1);
}
...
pub fn record_anonymization_applied(rule_name: &str) {
    m::counter!(
        "stellarclassic_pulse_anonymization_applied_total",
...
pub fn record_pii_detected(field: &str) {
    m::counter!(
        "stellarclassic_pulse_pii_detected_total",
        "field" => field.to_string()
    )
    .increment(1);
}
...
pub fn record_notification_maintenance_suppressed() {
    m::counter!("stellarclassic_pulse_notifications_maintenance_suppressed_total").increment(1);
}
...
pub fn set_channel_health(channel_name: &str, channel_type: &str, healthy: bool) {
    m::gauge!(
        "stellarclassic_pulse_notification_channel_healthy",
...
pub fn record_slow_query(query_type: &str) {
    m::counter!("stellarclassic_pulse_slow_queries_total", "query_type" => query_type.to_string())
        .increment(1);
}
...
pub fn record_query_duration(query_type: &str, duration: std::time::Duration) {
    m::histogram!(
        "stellarclassic_pulse_query_duration_seconds",
...
pub fn update_db_pool_metrics(pool: &PgPool) {
    m::gauge!("stellarclassic_pulse_db_pool_size").set(pool.size() as f64);
    m::gauge!("stellarclassic_pulse_db_pool_idle").set(pool.num_idle() as f64);
}
            ...
            if let Some(rest) = line.strip_prefix("VmRSS:") {
                if let Some(kb_str) = rest.split_whitespace().next() {
                    if let Ok(kb) = kb_str.parse::<u64>() {
                        m::gauge!("stellarclassic_pulse_process_memory_bytes").set((kb * 1024) as f64);
                    }
                }
                break;
            }
...
pub fn record_notification_delivery_latency(channel: &str, latency_seconds: f64) {
    m::histogram!(
        "stellarclassic_pulse_notification_delivery_latency_seconds",
...
pub fn update_notification_rate_per_minute(channel: &str, rate: f64) {
    m::gauge!(
        "stellarclassic_pulse_notification_rate_per_minute",
        "channel" => channel.to_string()
    )
    .set(rate);
}
...
pub fn record_notification_delivery_success() {
    m::counter!("stellarclassic_pulse_notification_delivery_success_total").increment(1);
}

/// #695: Record a failed notification delivery attempt.
pub fn record_notification_delivery_failure() {
    m::counter!("stellarclassic_pulse_notification_delivery_failure_total").increment(1);
}

/// #695: Record a successful webhook delivery.
pub fn record_webhook_delivery_success() {
    m::counter!("stellarclassic_pulse_webhook_delivery_success_total").increment(1);
}
...
pub fn update_contract_event_count(contract_id: &str, count: i64) {
    m::gauge!(
        "stellarclassic_pulse_contract_event_count",
        "contract_id" => contract_id.to_string()
    )
    .set(count as f64);
}
...
pub fn record_indexer_lag_observation(lag: u64) {
    m::histogram!("stellarclassic_pulse_indexer_lag_observation_ledgers").record(lag as f64);
}
...
pub fn record_abi_cache_hit(contract_id: &str) {
    m::counter!(
        "stellarclassic_pulse_abi_cache_hits_total",
        "contract_id" => contract_id.to_string()
    )
    .increment(1);
}
...
pub fn record_abi_cache_miss(contract_id: &str) {
    m::counter!(
        "stellarclassic_pulse_abi_cache_misses_total",
        "contract_id" => contract_id.to_string()
    )
    .increment(1);
}
...
pub fn record_abi_validation_failure(contract_id: &str) {
    m::counter!(
        "stellarclassic_pulse_abi_validation_failures_total",
        "contract_id" => contract_id.to_string()
    )
    .increment(1);
}
...
pub fn record_abi_cache_eviction() {
    m::counter!("stellarclassic_pulse_abi_cache_evictions_total").increment(1);
}
...
pub fn record_ledger_hash_mismatch(ledger: u64) {
    m::counter!(
        "stellarclassic_pulse_ledger_hash_mismatches_total",
...
pub fn record_ledger_hash_verified() {
    m::counter!("stellarclassic_pulse_ledger_hashes_verified_total").increment(1);
}

pub fn update_ledger_hash_chain_height(ledger: u64) {
    m::gauge!("stellarclassic_pulse_ledger_hash_chain_height").set(ledger as f64);
}
...
pub fn update_network_health(chain_id: &str, healthy: bool) {
    m::gauge!(
        "stellarclassic_pulse_network_healthy",
...
pub fn update_network_latest_ledger(chain_id: &str, ledger: u64) {
    m::gauge!(
        "stellarclassic_pulse_network_latest_ledger",
        "chain_id" => chain_id.to_string()
    )
    .set(ledger as f64);
}
...
pub fn record_network_indexer_error(chain_id: &str) {
    m::counter!(
        "stellarclassic_pulse_network_indexer_errors_total",
        "chain_id" => chain_id.to_string()
    )
    .increment(1);
}
...
pub fn record_compression_ratio(original_bytes: usize, compressed_bytes: usize) {
    if original_bytes > 0 {
        ...
        m::histogram!("stellarclassic_pulse_compression_ratio").record(ratio);
    }
    m::counter!("stellarclassic_pulse_events_compressed_total").increment(1);
    m::counter!(
        "stellarclassic_pulse_compression_bytes_saved_total"
    )
    .increment(original_bytes.saturating_sub(compressed_bytes) as u64);
}

pub fn record_decompression_failure() {
    m::counter!("stellarclassic_pulse_decompression_failures_total").increment(1);
}
...
pub fn record_http_compression_outcome(compressed: bool) {
    if compressed {
        m::counter!("stellarclassic_pulse_http_compression_applied_total").increment(1);
    } else {
        m::counter!("stellarclassic_pulse_http_compression_bypassed_total").increment(1);
    }
}
...
pub fn record_pagination_strategy(cursor: bool) {
    ...
    m::counter!("stellarclassic_pulse_pagination_requests_total", "strategy" => strategy).increment(1);
}
...
pub fn record_query_cache_hit(query_type: &str) {
    m::counter!(
        "stellarclassic_pulse_query_cache_hits_total",
        "query_type" => query_type.to_string()
    )
    .increment(1);
}
...
pub fn record_query_cache_miss(query_type: &str) {
    m::counter!(
        "stellarclassic_pulse_query_cache_misses_total",
        "query_type" => query_type.to_string()
    )
    .increment(1);
}
...
pub fn record_streaming_response_item_sent() {
    m::counter!("stellarclassic_pulse_streaming_response_items_sent_total").increment(1);
}

/// Record streaming response completed with item count
pub fn record_streaming_response_completed(item_count: u64) {
    m::counter!("stellarclassic_pulse_streaming_responses_completed_total").increment(1);
    m::histogram!("stellarclassic_pulse_streaming_response_items_per_stream").record(item_count as f64);
}

/// Record streaming response error
pub fn record_streaming_response_error(error_type: &str) {
    m::counter!(
        "stellarclassic_pulse_streaming_response_errors_total",
...
pub fn record_streaming_response_chunk(bytes: u64) {
    m::counter!("stellarclassic_pulse_streaming_response_chunks_total").increment(1);
    m::histogram!("stellarclassic_pulse_streaming_response_chunk_bytes").record(bytes as f64);
}
...
pub fn record_streaming_response_backpressure() {
    m::counter!("stellarclassic_pulse_streaming_response_backpressure_total").increment(1);
}

/// Record a stream that ended early. `reason` is `caller` or `client`.
pub fn record_streaming_response_cancelled(reason: &str) {
    m::counter!(
        "stellarclassic_pulse_streaming_responses_cancelled_total",
...
pub fn record_streaming_response_duration(seconds: f64) {
    m::histogram!("stellarclassic_pulse_streaming_response_duration_seconds").record(seconds);
}
...
pub fn record_query_stream_row() {
    m::counter!("stellarclassic_pulse_query_stream_rows_total").increment(1);
}

/// Record a completed batch fetch and how many rows it returned.
pub fn record_query_stream_batch(rows: u64) {
    m::counter!("stellarclassic_pulse_query_stream_batches_total").increment(1);
    m::histogram!("stellarclassic_pulse_query_stream_batch_rows").record(rows as f64);
}

/// Record a failed batch fetch.
pub fn record_query_stream_error() {
    m::counter!("stellarclassic_pulse_query_stream_batch_errors_total").increment(1);
}

/// Record a keep-alive tick emitted while a batch was still running.
pub fn record_query_stream_keepalive() {
    m::counter!("stellarclassic_pulse_query_stream_keepalives_total").increment(1);
}

/// Record a stream stopped by its caller.
pub fn record_query_stream_cancelled() {
    m::counter!("stellarclassic_pulse_query_streams_cancelled_total").increment(1);
}

/// Record a stream that stopped at `max_batches` with rows still unread.
pub fn record_query_stream_truncated() {
    m::counter!("stellarclassic_pulse_query_streams_truncated_total").increment(1);
}

/// Record a stream that delivered its whole result set.
pub fn record_query_stream_completed(rows: u64) {
    m::counter!("stellarclassic_pulse_query_streams_completed_total").increment(1);
    m::histogram!("stellarclassic_pulse_query_stream_rows_per_stream").record(rows as f64);
}
...
pub fn record_serialization_cache_miss(entity_type: &str) {
    m::counter!(
        "stellarclassic_pulse_serialization_cache_misses_total",
        "entity_type" => entity_type.to_string()
    )
    .increment(1);
}
...
pub fn record_serialization_time(entity_type: &str, duration_us: u64) {
    m::histogram!(
        "stellarclassic_pulse_serialization_time_us",
        "entity_type" => entity_type.to_string()
    )
...
pub fn record_serialization_cache_eviction(entity_type: &str) {
    m::counter!(
        "stellarclassic_pulse_serialization_cache_evictions_total",
        "entity_type" => entity_type.to_string()
    )
    .increment(1);
}
...
pub fn record_serialization_cache_invalidation(entity_type: &str, strategy: &str) {
    m::counter!(
        "stellarclassic_pulse_serialization_cache_invalidations_total",
        "entity_type" => entity_type.to_string(),
...
pub fn record_serialization_cache_prewarm(entity_type: &str, entries: u64) {
    m::counter!(
        "stellarclassic_pulse_serialization_cache_prewarmed_total",
        "entity_type" => entity_type.to_string()
    )
    .increment(entries);
}
...
pub fn record_serialization_cache_bytes_saved(entity_type: &str, bytes: u64) {
    m::counter!(
        "stellarclassic_pulse_serialization_cache_bytes_saved_total",
        "entity_type" => entity_type.to_string()
    )
    .increment(bytes);
}
...
pub fn update_serialization_cache_entry_count(count: u64) {
    m::gauge!("stellarclassic_pulse_serialization_cache_entry_count").set(count as f64);
}
...
pub fn update_serialization_cache_hit_rate(entity_type: &str, rate: f64) {
    m::gauge!(
        "stellarclassic_pulse_serialization_cache_hit_rate",
        "entity_type" => entity_type.to_string()
    )
    .set(rate);
}
...
pub fn update_serialization_cache_version(entity_type: &str, version: u64) {
    m::gauge!(
        "stellarclassic_pulse_serialization_cache_version",
        "entity_type" => entity_type.to_string()
    )
    .set(version as f64);
}
...
pub fn record_query_plan_cache_hit() {
    m::counter!("stellarclassic_pulse_query_plan_cache_hits_total").increment(1);
}

/// Record query plan cache miss
pub fn record_query_plan_cache_miss() {
    m::counter!("stellarclassic_pulse_query_plan_cache_misses_total").increment(1);
}

/// Record query plan cached
pub fn record_query_plan_cached() {
    m::counter!("stellarclassic_pulse_query_plans_cached_total").increment(1);
}

/// Record query planning time in milliseconds
pub fn record_query_planning_time(planning_time_ms: f64) {
    m::histogram!("stellarclassic_pulse_query_planning_time_ms").record(planning_time_ms);
}

/// Record a query plan eviction from the LRU/TTL cache. (#802)
pub fn record_query_plan_cache_eviction() {
    m::counter!("stellarclassic_pulse_query_plan_cache_evictions_total").increment(1);
}
...
pub fn update_query_plan_hit_ratio(ratio: f64) {
    ...
    m::gauge!("stellarclassic_pulse_query_plan_cache_hit_ratio").set(safe);
}

/// Update the live entry-count gauge after each insert. (#802)
pub fn update_query_plan_cache_entry_count(count: u64) {
    m::gauge!("stellarclassic_pulse_query_plan_cache_entry_count").set(count as f64);
}
...
pub fn update_schema_unused_indexes(count: u64) {
    m::gauge!("stellarclassic_pulse_schema_unused_indexes_total").set(count as f64);
}
...
pub fn update_schema_missing_future_partitions(count: u64) {
    m::gauge!("stellarclassic_pulse_schema_missing_future_partitions").set(count as f64);
}
...
pub fn record_advisory_lock_acquired(lock_id: i64) {
    m::counter!(
        "stellarclassic_pulse_advisory_lock_acquired_total",
...
pub fn record_advisory_lock_released(lock_id: i64) {
    m::counter!(
        "stellarclassic_pulse_advisory_lock_released_total",
        "lock_id" => lock_id.to_string()
    )
    .increment(1);
}
...
pub fn record_advisory_lock_retry(lock_id: i64) {
    m::counter!(
        "stellarclassic_pulse_advisory_lock_retries_total",
        "lock_id" => lock_id.to_string()
    )
    .increment(1);
}
...
pub fn record_advisory_lock_timeout(lock_id: i64) {
    m::counter!(
        "stellarclassic_pulse_advisory_lock_timeouts_total",
        "lock_id" => lock_id.to_string()
    )
    .increment(1);
}
...
pub fn record_advisory_lock_error(lock_id: i64) {
    m::counter!(
        "stellarclassic_pulse_advisory_lock_errors_total",
        "lock_id" => lock_id.to_string()
    )
    .increment(1);
}
...
pub fn record_advisory_lock_release_error(lock_id: i64) {
    m::counter!(
        "stellarclassic_pulse_advisory_lock_release_errors_total",
        "lock_id" => lock_id.to_string()
    )
    .increment(1);
}
...
pub fn record_priority_dequeue(priority: &str, wait_ms: u64) {
    m::counter!(
        "stellarclassic_pulse_webhook_priority_dequeued_total",
        "priority" => priority.to_string()
    )
    .increment(1);
    m::histogram!(
        "stellarclassic_pulse_webhook_priority_wait_ms",
        "priority" => priority.to_string()
    )
...
pub fn record_priority_violation(priority: &str) {
    m::counter!(
        "stellarclassic_pulse_webhook_priority_violations_total",
        "priority" => priority.to_string()
    )
    .increment(1);
}
...
pub fn record_webhook_signature_created(key_id: &str) {
    m::counter!(
        "stellarclassic_pulse_webhook_signatures_created_total",
...
pub fn record_webhook_signature_verified(key_id: &str, success: bool) {
    m::counter!(
        "stellarclassic_pulse_webhook_signature_verifications_total",
        "key_id" => key_id.to_string(),
...
pub fn record_http_cache_result(resource: &str, hit: bool) {
    m::counter!(
        "stellarclassic_pulse_http_cache_results_total",
...
pub fn record_postgres_health_ok(response_time_ms: u64) {
    m::counter!("stellarclassic_pulse_postgres_health_checks_total", "status" => "ok").increment(1);
    m::histogram!("stellarclassic_pulse_postgres_health_check_duration_ms").record(response_time_ms as f64);
}

/// Record degraded PostgreSQL health check
pub fn record_postgres_health_degraded(response_time_ms: u64) {
    m::counter!("stellarclassic_pulse_postgres_health_checks_total", "status" => "degraded").increment(1);
    m::histogram!("stellarclassic_pulse_postgres_health_check_duration_ms").record(response_time_ms as f64);
}

/// Record failed PostgreSQL health check
pub fn record_postgres_health_error(response_time_ms: u64) {
    m::counter!("stellarclassic_pulse_postgres_health_checks_total", "status" => "error").increment(1);
    m::histogram!("stellarclassic_pulse_postgres_health_check_duration_ms").record(response_time_ms as f64);
}

/// Record successful RPC health check
pub fn record_rpc_health_ok(response_time_ms: u64) {
    m::counter!("stellarclassic_pulse_rpc_health_checks_total", "status" => "ok").increment(1);
    m::histogram!("stellarclassic_pulse_rpc_health_check_duration_ms").record(response_time_ms as f64);
}

/// Record failed RPC health check
pub fn record_rpc_health_error(response_time_ms: u64) {
    m::counter!("stellarclassic_pulse_rpc_health_checks_total", "status" => "error").increment(1);
    m::histogram!("stellarclassic_pulse_rpc_health_check_duration_ms").record(response_time_ms as f64);
}

/// Record a successful batch delivery of N events.
pub fn record_batch_delivered(count: u64) {
    m::counter!("stellarclassic_pulse_batch_delivered_total").increment(count);
    m::counter!("stellarclassic_pulse_batch_deliveries_total").increment(1);
}

/// Record a failed batch delivery attempt.
pub fn record_batch_delivery_failed() {
    m::counter!("stellarclassic_pulse_batch_delivery_failures_total").increment(1);
}

/// Record a batch config update.
pub fn record_batch_config_updated() {
    m::counter!("stellarclassic_pulse_batch_config_updates_total").increment(1);
}
...
pub fn record_conditional_get_304() {
    m::counter!("stellarclassic_pulse_conditional_get_304_total").increment(1);
}

/// Record bandwidth savings from conditional GETs.
pub fn record_conditional_get_bandwidth_saved(bytes: u64) {
    m::counter!("stellarclassic_pulse_conditional_get_bandwidth_saved_bytes_total").increment(bytes);
}

/// Record ETag cache hit.
pub fn record_etag_cache_hit() {
    m::counter!("stellarclassic_pulse_etag_cache_hits_total").increment(1);
}

/// Record ETag cache miss.
pub fn record_etag_cache_miss() {
    m::counter!("stellarclassic_pulse_etag_cache_misses_total").increment(1);
}
...
pub fn record_subscription_paused() {
    m::counter!("stellarclassic_pulse_subscriptions_paused_total").increment(1);
}

/// Record subscription resume event.
pub fn record_subscription_resumed() {
    m::counter!("stellarclassic_pulse_subscriptions_resumed_total").increment(1);
}

/// Record auto-resume of paused subscriptions.
pub fn record_subscription_auto_resumed(count: u64) {
    m::counter!("stellarclassic_pulse_subscriptions_auto_resumed_total").increment(count);
}
...
pub fn record_anomaly_detection_configured(count: u64) {
    m::counter!("stellarclassic_pulse_anomaly_detection_configured_total").increment(count);
}

/// Record anomaly alerts retrieved.
pub fn record_anomaly_alerts_queried(count: u64) {
    m::counter!("stellarclassic_pulse_anomaly_alerts_queried_total").increment(count);
}

/// Record anomaly alert acknowledged.
pub fn record_anomaly_alert_acknowledged() {
    m::counter!("stellarclassic_pulse_anomaly_alerts_acknowledged_total").increment(1);
}

/// Record anomaly score observation.
pub fn record_anomaly_score(metric_name: &str, score: f64) {
    m::histogram!(
        "stellarclassic_pulse_anomaly_score",
...
pub fn record_anomaly_threshold_crossed(metric_name: &str, severity: &str) {
    m::counter!(
        "stellarclassic_pulse_anomaly_threshold_crossings_total",
        "metric" => metric_name.to_string(),
...
pub fn update_fd_count(count: u64) {
    m::gauge!("stellarclassic_pulse_fd_count").set(count as f64);
}

/// Update disk I/O read bytes gauge
pub fn update_disk_read_bytes(bytes: u64) {
    m::gauge!("stellarclassic_pulse_disk_read_bytes_total").set(bytes as f64);
}

/// Update disk I/O write bytes gauge
pub fn update_disk_write_bytes(bytes: u64) {
    m::gauge!("stellarclassic_pulse_disk_write_bytes_total").set(bytes as f64);
}

/// Update disk read syscall count gauge
pub fn update_disk_syscalls_read(count: u64) {
    m::gauge!("stellarclassic_pulse_disk_syscalls_read_total").set(count as f64);
}

/// Update disk write syscall count gauge
pub fn update_disk_syscalls_write(count: u64) {
    m::gauge!("stellarclassic_pulse_disk_syscalls_write_total").set(count as f64);
}

/// Update process memory RSS gauge
pub fn update_memory_rss_bytes(bytes: u64) {
    m::gauge!("stellarclassic_pulse_process_memory_rss_bytes").set(bytes as f64);
}

/// Update process memory VMS gauge
pub fn update_memory_vms_bytes(bytes: u64) {
    m::gauge!("stellarclassic_pulse_process_memory_vms_bytes").set(bytes as f64);
}
...
pub fn update_backup_size_bytes(bytes: u64) {
    m::gauge!("stellarclassic_pulse_backup_size_bytes").set(bytes as f64);
}
...
pub fn record_trace_span_created(span_name: &str) {
    m::counter!(
        "stellarclassic_pulse_trace_spans_created_total",
...
pub fn record_trace_sampled(sampled: bool) {
    m::counter!(
        "stellarclassic_pulse_trace_samples_total",
...
pub fn record_error_rate(endpoint: &str, rate: f64) {
    m::gauge!(
        "stellarclassic_pulse_sli_error_rate",
...
pub fn record_availability(endpoint: &str, availability: f64) {
    m::gauge!(
        "stellarclassic_pulse_sli_availability",
        "endpoint" => endpoint.to_string()
    )
    .set(availability);
}
...
pub fn update_slo_budget_burndown(slo: &str, consumed: f64) {
    m::gauge!(
        "stellarclassic_pulse_slo_budget_burndown",
        "slo" => slo.to_string()
    )
    .set(consumed);
}
...
pub fn record_alert_fired(alert_name: &str, severity: &str) {
    m::counter!(
        "stellarclassic_pulse_alerts_fired_total",
...
pub fn record_alert_resolved(alert_name: &str) {
    m::counter!(
        "stellarclassic_pulse_alerts_resolved_total",
        "alert_name" => alert_name.to_string()
    )
    .increment(1);
}
...
pub fn record_alert_silenced(alert_name: &str, duration_minutes: u64) {
    m::counter!(
        "stellarclassic_pulse_alerts_silenced_total",
        "alert_name" => alert_name.to_string()
    )
    .increment(1);
    m::gauge!(
        "stellarclassic_pulse_alert_silence_duration_minutes",
        "alert_name" => alert_name.to_string()
    )
...
pub fn update_active_alerts_count(component: &str, count: u64) {
    m::gauge!(
        "stellarclassic_pulse_active_alerts",
        "component" => component.to_string()
    )
    .set(count as f64);
}
